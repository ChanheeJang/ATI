...
CRect myZoomInfo::getZoomedRect(CPoint currentCtr)
{
	...
	CPoint newCntrPt = checkCntrPt(currentCtr);
...
CPoint myZoomInfo::checkCntrPt(CPoint currentCtr)
{
	CPoint newCntrPt = currentCtr;
	if (currentCtr.x - m_szImageSize.cx / (2 * m_nZoomX) < 1)	newCntrPt.x = m_szImageSize.cx / (2 * m_nZoomX);
	if (currentCtr.y - m_szImageSize.cy / (2 * m_nZoomX) < 1)	newCntrPt.y = m_szImageSize.cy / (2 * m_nZoomX);
	
	if (currentCtr.x + m_szImageSize.cx / (2 * m_nZoomX) > m_szImageSize.cx - 1)	newCntrPt.x = m_szImageSize.cx - m_szImageSize.cx / (2 * m_nZoomX);
	if (currentCtr.y + m_szImageSize.cy / (2 * m_nZoomX) > m_szImageSize.cy - 1)	newCntrPt.y = m_szImageSize.cy - m_szImageSize.cy / (2 * m_nZoomX);
