...
private:
	int m_iSrcWidth;
	int m_iSrcHeight;
	float m_fXScale; //=imgSize / frameSize
	float m_fYScale; //=imgSize / frameSize
	double m_dSrcRatio; //m_dSrcRatio =(double)srcSize.cx / (double)srcSize.cy;
	CSize m_szSrcSize;
	CRect m_rcSrcRect;  //Rect that encompasses src image
	CPoint m_ptSrcCenter;

	CSize m_szFrameSize;
	CRect m_rcFrameRect; //Rect in which src image will be displayed
