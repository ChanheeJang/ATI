...
//	dc.SelectObject(&pen);
//	
//	CRect imgRect=	pDoc->myImage.getZoomedRect();
...
BOOL myDial::OnEraseBkgnd(CDC* pDC)
{
	...
	imgRect = pDoc->myImage.getZoomedRect();
	imgRect.left *= prev2srcRatioX;
	imgRect.right *= prev2srcRatioX;
	imgRect.top *= prev2srcRatioY;
	imgRect.bottom *= prev2srcRatioY;
...
void myDial::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	pDoc->myImage.EnablePreviewShift = true;
	...
	CSize srcSize = pDoc->myImage.GetSrcSize();
	CPoint newCenter = pDoc->myImage.checkCntrPt(CPoint(pDoc->myImage.GetSrcZoomCenter().x + xDiff, pDoc->myImage.GetSrcZoomCenter().y + yDiff), srcSize);
	shiftedSrcRect = CRect(newCenter.x - srcSize.cx / 2, newCenter.y - srcSize.cy / 2, newCenter.x + srcSize.cx / 2 + 1, newCenter.y + srcSize.cy / 2 + 1);

	pDoc->myImage.SetSrcZoomCenter(shiftedSrcRect);
...
void myDial::OnMouseMove(UINT nFlags, CPoint point)
{
	if (LDown)
	{
		CTask2View* pView = (CTask2View*)((CMainFrame*)AfxGetApp()->m_pMainWnd)->GetActiveView();
		pDoc->myImage.DisableAllOptions();
		pDoc->myImage.EnablePreviewDrag = true;
		...
		CPoint newCenter = pDoc->myImage.checkCntrPt(CPoint(pDoc->myImage.GetSrcZoomCenter().x + dragNowPt.x, pDoc->myImage.GetSrcZoomCenter().y + dragNowPt.y), srcSize);
