...
BOOL myDial::OnEraseBkgnd(CDC* pDC)
{
	if (!isBGresized)
	{
		...
		destDC->CreateCompatibleDC(pDC);  //destDC : resized DC
...
void myDial::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	CSize previewFrameSize = CSize(imgRect.Width(), imgRect.Height());
	CPoint newCenter = pDoc->myImage->checkCntrPt(CPoint(pDoc->myImage->GetSrcZoomCenter().x + xDiff, pDoc->myImage->GetSrcZoomCenter().y + yDiff), previewFrameSize);
...
void myDial::OnMouseMove(UINT nFlags, CPoint point)
{
	if (LDown)
	{
		...
		CPoint newCenter = pDoc->myImage->checkCntrPt(CPoint(pDoc->myImage->GetSrcZoomCenter().x + dragNowPt.x, pDoc->myImage->GetSrcZoomCenter().y + dragNowPt.y), previewFrameSize);
